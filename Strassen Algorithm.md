...
## 슈트라센 원리
   ...
   `n by n`을 쪼개서 `2 by 2` 까지 가면 7번의 곱셈과 18번의 덧셈으로 표현이 가능한데 이는 기존 방식보다 곱셈이 적다 그러므로 엄청 큰 행렬이 들어와서 n by n 을 많이 한다 가정하면 속도가 엄청    빨라진다.
...
입력 값을 128넣어주었는데 성능차이가 엄청나기 시작한다. 숫자가 커지면 커질수록 더 성능차이가 날 것이다.


사실 입력 값을 작게 넣었을 때는 크게 차이가 안난다. 
